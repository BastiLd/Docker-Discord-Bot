...
- `docker-compose.yml` with a stable app `name`, the published image `ghcr.io/bastild/docker-discord-bot:0.1.2`, `WEBUI_PORT`, `/DATA/AppData/$AppID/...` bind mounts and `x-casaos` metadata.
...
- Tag: `0.1.2` for a fixed release, or `main` if you want to pull the newest pushed build during private testing.
...
1. Confirm the versioned multi-arch image exists at `ghcr.io/bastild/docker-discord-bot:0.1.2`.
